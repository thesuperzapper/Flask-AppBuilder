...
class LDAPSearchTestCase(unittest.TestCase):
    ...
    call_initialize = (
        "initialize",
        tuple(["ldap://localhost/"]),
        {}
    )

    call_set_option = (
        "set_option",
        tuple([ldap.OPT_REFERRALS, 0]),
        {}
    )
    call_bind_admin = (
        "simple_bind_s",
    ...
    call_search_alice_memberof = (
        "search_s",
        tuple(["ou=users,o=test", 2, "(uid=alice)", ["givenName", "sn", "email", "memberOf"]]),
        {},
    )
    call_search_alice_filter = (
        "search_s",
        tuple(["ou=users,o=test", 2, "(&(memberOf=cn=staff,ou=groups,o=test)(uid=alice))",
               ["givenName", "sn", "email"]]),
        {},
    )
    ...
    def test___search_ldap_filter(self):
        ...
        """
        # MockLdap needs non-bytes for search filters, so we patch `memberOf` to a string, only for this test
        with patch.dict(
            ...
            self.app.config["AUTH_LDAP_BIND_PASSWORD"] = "admin_password"
            self.app.config["AUTH_LDAP_SEARCH"] = "ou=users,o=test"
            self.app.config["AUTH_LDAP_SEARCH_FILTER"] = "(memberOf=cn=staff,ou=groups,o=test)"
            self.appbuilder = AppBuilder(self.app, self.db.session)
            sm = self.appbuilder.sm
    ...
    def test__indirect_bind__unregistered__no_search(self):
        ...
        # validate - user was NOT allowed to log in (because indirect bind requires search)
        self.assertIsNone(user)
