...
You can give FlaskAppBuilder roles based on LDAP roles (note, this requires AUTH_LDAP_SEARCH to be set)::
    ...
    # a mapping from LDAP DN to list of FAB roles
    AUTH_ROLES_MAPPING = {
        "cn=fab_users,ou=groups,dc=example,dc=com": ["User"],
        "cn=fab_admins,ou=groups,dc=example,dc=com": ["Admin"],
    }

    # the LDAP user attribute which has their role DN's
...
To customize the user information retrieval, you can create your own method like this::
...
Your method should return a dictionary with the user information, keys having the same column names as the User Model.
